...
import { runRISequenceMap } from "@/lib/api"
...
async function downloadPdf(url: string, motifLabel: string) {
  ...
  pdf.save(`rnapeaks-ri-${motifLabel}.pdf`)
}

const ALL_GROUPS = ["Retained", "Excluded", "Control"] as const

export function RISequenceMapTab() {
  ...
  async function handleRun() {
    ...
    try {
      if (motifMode === "combined") {
        const url = await runRISequenceMap({
          ...commonParams,
      ...
      >
        ...
        <div className="border-t px-5 py-4">
          ...
          >
            ...
            {loading ? "Running…" : "Run RI Sequence Map"}
          </Button>
        </div>
      </form>
              ...
              <div className="flex gap-2">
                <Button
                  ...
                  onClick={() =>
                    downloadPng(
                      curImage!.url,
                      `rnapeaks-ri-${curImage!.label}.png`
                    )
                  }
                >
