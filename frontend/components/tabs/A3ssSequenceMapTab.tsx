...
import { runA3ssSequenceMap } from "@/lib/api"
...
async function downloadPdf(url: string, motifLabel: string) {
  ...
  pdf.save(`rnapeaks-a3ss-${motifLabel}.pdf`)
}

const ALL_GROUPS = ["Retained", "Excluded", "Control"] as const

export function A3ssSequenceMapTab() {
  ...
  async function handleRun() {
    ...
    try {
      if (motifMode === "combined") {
        const url = await runA3ssSequenceMap({
          ...commonParams,
    ...
    <div className="flex h-full">
      ...
      >
        ...
        <div className="border-b px-5 py-3.5">
          <p className="text-sm font-semibold tracking-tight">3&apos; Sequence Map</p>
          <p className="mt-0.5 text-xs text-muted-foreground">
            Motif density around alternative 3&apos; splice-site events
          </p>
        </div>
        ...
        <div className="min-h-0 flex-1 space-y-4 overflow-y-auto px-5 py-4">
          ...
          {!matsUploadId && (
            <p className="-mt-2 text-[11px] text-muted-foreground">
              No file selected - sample A3SS.MATS data will be used
            </p>
          )}
        ...
        <div className="border-t px-5 py-4">
          ...
          >
            ...
            {loading ? "Running…" : "Run 3' Sequence Map"}
          </Button>
        </div>
      </form>
              ...
              <div className="flex gap-2">
                <Button
                  ...
                  onClick={() =>
                    downloadPng(
                      curImage!.url,
                      `rnapeaks-a3ss-${curImage!.label}.png`
                    )
                  }
                >
